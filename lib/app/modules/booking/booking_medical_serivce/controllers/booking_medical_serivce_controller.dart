...
class BookingMedicalSerivceController extends BaseController {
  ...
  Rx<MedicalService> selectedMedicalService = MedicalService(
          medicalSpecialtyName: '', medicalServiceName: 'Vui lòng chọn dịch vụ')
      .obs;
  Rx<DateTime> selectedDate = DateTime.now().add(const Duration(days: 3)).obs;
  ...
  getDataService(TypeService type) async {
    switch (type) {
      case TypeService.labExam:
        await doctorApi.getListMedicalServiceExamination().then((value) {
        ...
        break;
      case TypeService.vaccination:
        await doctorApi.getListMedicalServiceVacination().then((value) {
          listService.value = value;
        }).catchError((error) {
          log("err:$error");
          isLockButton(false);
        ...
        break;
      case TypeService.psychological:
        await doctorApi.getListMedicalServicePyschological().then((value) {
          listService.value = value;
        }).catchError((error) {
          log("err:$error");
          isLockButton(false);
  ...
  onTapClinic(Clinic clinic) async {
    selectedMedicalService.value = MedicalService(
        medicalSpecialtyName: '', medicalServiceName: 'Vui lòng chọn dịch vụ');
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      listService[index].id == selectedMedicalService.value.id
                          ? const Icon(Icons.check)
                          : const SizedBox.shrink()
                    ],
                  ),
  ...
  setTimeSlotChoice(DateTime dateSelected, DutySchedule slot) async {
    ...
    requestParamBooking.dutySchedule = slot;
